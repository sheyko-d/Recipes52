...
public class LineChart extends XYChart {
			...
			if (fill.getType() != FillOutsideLine.Type.NONE) {
				...
				if (fill.getType() == FillOutsideLine.Type.BOUNDS_ABOVE
					...
					for (int i = 3; i < fillPoints.size(); i += 2) {
						...
						if (prevValue < referencePoint
								&& value > referencePoint
								|| prevValue > referencePoint
								&& value < referencePoint) {
							float prevX = fillPoints.get(i - 3);
							float x = fillPoints.get(i - 1);
							boundsPoints.add(prevX + (x - prevX)
									* (referencePoint - prevValue)
									/ (value - prevValue));
	...
	protected ClickableArea[] clickableAreasForPoints(List<Float> points,
		...
		for (int i = 0; i < length; i += 2) {
			...
			ret[i / 2] = new ClickableArea(new RectF(points.get(i)
					- selectableBuffer, points.get(i + 1) - selectableBuffer,
					points.get(i) + selectableBuffer, points.get(i + 1)
							+ selectableBuffer), values.get(i),
					values.get(i + 1));
		}
		return ret;
	}
