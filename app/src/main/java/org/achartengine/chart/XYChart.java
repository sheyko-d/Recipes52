...
public abstract class XYChart extends AbstractChart {
	...
	public void draw(Canvas canvas, int x, int y, int width, int height,
		...
		if (paint.getTypeface() == null
				|| (mRenderer.getTextTypeface() != null && paint.getTypeface()
						.equals(mRenderer.getTextTypeface()))
		...
		drawXLabels_dup(
				getValidLabels(getXLabels(minX[0], maxX[0],
						mRenderer.getXLabels())),
				mRenderer.getXTextLabelLocations(), canvas, paint, left, top,
				...
				if (count > 0) {
					...
					for (int j = 0; j < count; j++) {
						...
						if (xS < (xS + bound.width())
								&& yS < canvas.getHeight()) {
							if (xS > mRenderer.getYLabelsPadding()
									+ paint.measureText(series
											.getAnnotationAt(j))) {
								drawString(canvas, series.getAnnotationAt(j),
										xS, yS, paint);
							}
						}
					}
		...
		if (showLabels || showGridX) {
			...
			drawXLabels(xLabels, mRenderer.getXTextLabelLocations(), canvas,
					...
					minX[0], maxX[0]);
			drawYLabels(allYLabels, canvas, paint, maxScaleNumber, left + 10,
					right, bottom, yPixelsPerUnit, minY);
				...
				if (or == Orientation.HORIZONTAL) {
					drawText(canvas, mRenderer.getXTitle(), x + width / 2,
							bottom + mRenderer.getLabelsTextSize() * 4 / 3
									+ mRenderer.getXLabelsPadding() + size,
							paint, 0);
					for (int i = 0; i < maxScaleNumber; i++) {
						Align axisAlign = mRenderer.getYAxisAlign(i);
						if (axisAlign == Align.LEFT) {
					...
					drawText(canvas, mRenderer.getXTitle(), x + width / 2, y
							+ height - size + mRenderer.getXLabelsPadding(),
							paint, -90);
		...
		if (or == Orientation.HORIZONTAL) {
			drawLegend(canvas, mRenderer, titles, left, right, y
					+ (int) mRenderer.getXLabelsPadding(), width, height,
	...
	protected void drawChartValuesText(Canvas canvas, XYSeries series,
		...
		if (points.size() > 1) { // there are more than one point
			...
			for (int k = 0; k < points.size(); k += 2) {
				if (k == 2) {
					// decide whether to display first two points'
					// values or
					// not
			...
			for (int k = 0; k < points.size(); k += 2) {
				drawText(
						...
						getLabel(renderer.getChartValuesFormat(),
								series.getY(startIndex + k / 2)),
						points.get(k),
						points.get(k + 1) - renderer.getChartValuesSpacing(),
						paint, 0);
			}
		}
	}
	...
	protected void drawXLabels_dup(List<Double> xLabels,
			Double[] xTextLabelLocations, Canvas canvas, Paint paint, int left,
			int top, int bottom, double xPixelsPerUnit, double minX, double maxX) {
		boolean showLabels = mRenderer.isShowLabels();
		drawXTextLabels_dup(xTextLabelLocations, canvas, paint, showLabels,
				...
				if (or == Orientation.HORIZONTAL) {
					if (showLabels && !textLabel) {
						...
						if (axisAlign == Align.LEFT) {
							...
							drawText(
									canvas,
									getLabel(mRenderer.getLabelFormat(), label),
	...
	protected void drawXTextLabels_dup(Double[] xTextLabelLocations,
			Canvas canvas, Paint paint, boolean showLabels, int left, int top,
