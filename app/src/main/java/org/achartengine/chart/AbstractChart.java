...
public abstract class AbstractChart implements Serializable {
		...
		if (renderer.isShowLegend()) {
			...
			for (int i = 0; i < sLength; i++) {
				...
				if (r.isShowLegendItem()) {
					...
					if (i > 0
							&& getExceed(currentWidth, renderer, right, width)) {
						currentX = left;
	...
	protected void drawPath(Canvas canvas, List<Float> points, Paint paint,
		...
		final int EXCEED_LIMIT = 60;
		tempDrawPoints = calculateDrawPoints(points.get(0), points.get(1),
				points.get(2), points.get(3), height, width);
		if (points.get(0) < EXCEED_LIMIT) {
			path.moveTo(EXCEED_LIMIT, tempDrawPoints[1]);
		} else {
		...
		for (int i = 4; i < length; i += 2) {
			...
			tempDrawPoints = calculateDrawPoints(points.get(i - 2),
	...
	protected void drawLabel(Canvas canvas, String labelText,
			...
			Paint paint, boolean line, boolean display) {
