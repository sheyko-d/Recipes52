	...
	public ReflectiveProperty(Class<T> propertyHolder, Class<V> valueType,
		...
		try {
			...
			try {
				 ...
				 * fallback.
				 */
				mGetter = propertyHolder.getDeclaredMethod(getterName,
						(Class<?>[]) null);
				...
				try {
					...
					try {
						 ...
						 * getDeclaredMethod(..) does not find superclass
						 * methods, so it's implemented as a fallback.
						 */
						mGetter = propertyHolder.getDeclaredMethod(getterName,
								(Class<?>[]) null);
						mGetter.setAccessible(true);
						...
						try {
							...
							// no way to access property - throw appropriate
							// exception
							throw new NoSuchPropertyException(
		...
		try {
			...
			// The native implementation uses JNI to do reflection, which allows
